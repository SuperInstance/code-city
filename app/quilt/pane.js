// pane.js — the "see the backend in production" toggle.
...
export class BackendPane {
  constructor(bridge, { tail = 14 } = {}) {
    this.bridge = bridge;
    this.tail = tail;
    this.visible = false;
    this.#buildDom();
    bridge.onCell(() => { if (this.visible) this.render(); });
  }
  ...
  #buildDom() {
    ...
    this.pane.querySelector('#quilt-export').addEventListener('click', () => this.exportWitness());
  ...
  render() {
    ...
    const vitals = this.pane.querySelector('#quilt-vitals');
    const ops = Object.entries(s.counters)
      .map(([op, n]) => `<span>${op} ${n}</span>`).join('');
    vitals.innerHTML = `
      <span>chain <strong>${s.cells}</strong></span>
      <span>coherence <strong>${s.coherence}</strong></span>
      <span class="dim">head ${s.head}…</span>${ops}`;
    const tail = this.pane.querySelector('#quilt-tail');
    tail.innerHTML = cells.slice(-this.tail).map(c =>
  ...
  exportWitness() {
    const blob = new Blob([this.bridge.kernel.witnessJsonl()], { type: 'application/jsonl' });
